	...
	function onRequest(context) {
		...
		if (context.request.method === "GET") {
			...
			var allLeadsActitivityReportingSearch = search.load({
				type: "customer",
				id: "customsearch_all_leads_reporting_20241_5",
			});

			if (!isNullorEmpty(date_range)) {
				weeklyHistoricalPulseJSONSearch.filters.push(
					search.createFilter({
						name: "name",
						join: null,
						operator: search.Operator.IS,
						values: lastWeeksFridayDateYYYYMMDD,
					})
				);
			}

			weeklyHistoricalPulseJSONSearch
				.run()
				.each(function (weeklyHistoricalPulseJSONResultSet) {
					var customerInternalId = weeklyHistoricalPulseJSONResultSet.getValue({
						name: "internalid",
					});
					var name = weeklyHistoricalPulseJSONResultSet.getValue({
						name: "name",
					});
					totalInboundJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_inbound_json",
						})
					);
					totalFranchiseeJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_franchisee_json",
						})
					);
					totalBAUJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_bau_json",
						})
					);
					totalBAUStatusChangeJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_bau_staus_change_json",
						})
					);
					totalSignUpJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_sign_up_json",
						})
					);
					totalCommencedJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_commencement_date_json",
						})
					);
					totalCancelledJSON.push(
						weeklyHistoricalPulseJSONResultSet.getValue({
							name: "custrecord_total_customer_lost_json",
						})
					);

					tableColumnsNames.push(name);

					return true;
				});

			log.audit({
				title: "tableColumnsNames.length",
				details: tableColumnsNames.length,
			});
			...
			var last2Rows = tableColumnsNames.length - 2;
