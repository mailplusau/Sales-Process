...
define([
	...
	var callForceSubCategoryStatusDataSet = [];
	...
	function loadDatatable(zee_id, userId) {
		...
		console.log("date_synced_from: " + date_synced_from);
		console.log("date_synced_to " + date_synced_to);
		...
		var showProspectPlusTasks = false;
		...
		console.log('pp_sync: ' + pp_sync);

		if (pp_sync == 1 || pp_sync == '1') {
			showProspectPlusTasks = true;
		}

		console.log('showCallForceTasks: ' + showCallForceTasks);
		console.log('showFranchiseeLeadsPieChart: ' + showFranchiseeLeadsPieChart);
		console.log('showIlliciumTasks: ' + showIlliciumTasks);
		console.log('showProspectPlusTasks: ' + showProspectPlusTasks);
		...
		if (showIlliciumTasks == true) {
			...
			//Illicium Campaign - Count by Sub-Category & Status
			var callForceLeadsCountBySubCategoryStatusSearch = search.load({
				type: "customer",
				id: "customsearch_illicium_count_outcome_st_2",
			});

			callForceLeadsCountBySubCategoryStatusSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var oldCallforceSubCategory = null;
			var countCallforceSyncSubCategoryCount = 0;
			...
			callForceLeadsCountBySubCategoryStatusSearch
				.run()
				.each(function (callForceLeadsCountBySubCategoryStatusSearchResultSet) {
					var prospectCount = parseInt(
						callForceLeadsCountBySubCategoryStatusSearchResultSet.getValue({
							name: "internalid",
							summary: "COUNT",
						})
					);

					var custStatus = callForceLeadsCountBySubCategoryStatusSearchResultSet.getValue(
						{
							name: "entitystatus",
							summary: "GROUP",
						}
					...
					var callForceSubCategory =
						callForceLeadsCountBySubCategoryStatusSearchResultSet.getText({
					...
					console.log("prospectCount: " + prospectCount);
					console.log("custStatus: " + custStatus);
					console.log("callforceOutcome: " + callForceSubCategory);

					if (
						countCallforceSyncSubCategoryCount != 0 &&
						callForceSubCategory != oldCallForceSubCategory
					) {
						callForceSubCategoryStatusDataSet.push({
							callOutcome: oldCallForceSubCategory,
							suspect_hot_lead: suspect_hot_lead,
							prospecy_quote_sent: prospecy_quote_sent,
							suspect_reassign: suspect_reassign,
							prospect_no_answer: prospect_no_answer,
					...
					oldCallForceSubCategory = callForceSubCategory;
					countCallforceSyncSubCategoryCount++;
					return true;
				});

			console.log(
				"countCallforceSyncSubCategoryCount: " + countCallforceSyncSubCategoryCount
			);
			if (countCallforceSyncSubCategoryCount > 0) {
				callForceSubCategoryStatusDataSet.push({
					callOutcome: oldCallForceSubCategory,
					suspect_hot_lead: suspect_hot_lead,
					prospecy_quote_sent: prospecy_quote_sent,
			...
			console.log(
				"callForceSubCategoryStatusDataSet" + callForceSubCategoryStatusDataSet
			);

			subCategoryStatusDataSet = [];
			if (!isNullorEmpty(callForceSubCategoryStatusDataSet)) {
				callForceSubCategoryStatusDataSet.forEach(function (preview_row, index) {
					var hotLeadPercentage = parseInt(
						(preview_row.suspect_hot_lead / preview_row.total_leads) * 100
					);
					var hotLeadCol =
			...
			var dataTableCallForceDateSyncedSubCategory = $(
				"#mpexusage-illiciumIndustrySubCategoryStatus"
			).DataTable({
				data: subCategoryStatusDataSet,
				pageLength: 250,
				order: [25, "desc"],
				responsive: true,
				layout: {
		...
		//!Creating a reporting tab based on the tasks created for leads synced with ProspectPlus
		if (showProspectPlusTasks == true) {
			console.log("Generating ProspectPlus Tasks Report");
			console.log('date_from: ' + date_from);
			console.log('date_to: ' + date_to);
			console.log('date_synced_from: ' + date_synced_from);
			console.log('date_synced_to: ' + date_synced_to);
			//Get the count of the list of leads sent to Illicium
			//ProspectPlus - Suspect Unqualified List
			var prospectplusLeadsListSearch = search.load({
				type: "customer",
				id: "customsearch_pp_unqualified_list",
			});

			prospectplusLeadsListSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var prospectplusLeadsListSearchCount = prospectplusLeadsListSearch.runPaged().count;

			//Get the count of leads that is Qualified but with No Appointment
			//NetSuite Search:ProspectPlus - Suspect Qualified List - With No Tasks
			var qualifiedPPLeadsListWithNoTasksSearch = search.load({
				type: "customer",
				id: "customsearch_pp_quali_list_no_tasks",
			});

			qualifiedPPLeadsListWithNoTasksSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var totalQualifiedPPLeadWithNoTasks = 0;

			qualifiedPPLeadsListWithNoTasksSearch
				.run()
				.each(function (qualifiedLeadsListSearchResult) {
					var custInternalID = qualifiedLeadsListSearchResult.getValue({
						name: "internalid",
					...
					if (illiciumAllTasksSearchCount == 0) {
						totalQualifiedPPLeadWithNoTasks++;
					}
					return true;
				});

			//Sales Reporting - ProspectPlus - With Tasks
			var prospectplusTasksSearch = search.load({
				type: "customer",
				id: "customsearch_pp_leads_with_tasks",
			});

			prospectplusTasksSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var countProspectPlusTasks = 0;
			var oldProspectPlusDate = null;

			var scheduledProspectPlusTasks = 0;
			var rescheduledProspectPlusTasks = 0;
			var completedProspectPlusTasks = 0;
			var totalProspectPlusTasks = 0;

			var totalCompletedTasks = 0;
			var totalScheduledTasks = 0;
			var totalRescheduledTasks = 0;

			prospectplusTasksSearch.run().each(function (illiciumTasksSearchResultSet) {
				var tasksCount = parseInt(
					illiciumTasksSearchResultSet.getValue({
						name: "internalid",
						join: "task",
				...
				if (countProspectPlusTasks == 0 || taskDate == oldProspectPlusDate) {
					if (taskTitle.replace(/\s+$/, "") == "Rescheduled Illicium Appointment") {
						rescheduledProspectPlusTasks = rescheduledProspectPlusTasks + tasksCount;
						totalRescheduledTasks = totalRescheduledTasks + tasksCount;
					} else {
						if (taskStatus == "Completed") {
							completedProspectPlusTasks = completedProspectPlusTasks + tasksCount;
							totalCompletedTasks = totalCompletedTasks + tasksCount;
						} else if (taskStatus == "Not Started") {
							scheduledProspectPlusTasks = scheduledProspectPlusTasks + tasksCount;
							totalScheduledTasks = totalScheduledTasks + tasksCount;
						}
					}

					totalProspectPlusTasks = totalProspectPlusTasks + tasksCount;
				} else if (taskDate != oldIlliciumDate) {
					var taskDueDate = convertToDateInputFormat(oldProspectPlusDate);

					prospectplusTasksDataSet.push([
						taskDueDate,
						scheduledProspectPlusTasks,
						rescheduledProspectPlusTasks,
						completedProspectPlusTasks,
						totalProspectPlusTasks,
					]);

					scheduledProspectPlusTasks = 0;
					rescheduledProspectPlusTasks = 0;
					completedProspectPlusTasks = 0;
					totalProspectPlusTasks = 0;
				...
				oldProspectPlusDate = taskDate;
				countProspectPlusTasks++;
				return true;
			});

			if (countProspectPlusTasks > 0) {
				var taskDueDate = convertToDateInputFormat(oldProspectPlusDate);
			...
			console.log("prospectplusTasksDataSet" + prospectplusTasksDataSet);

			var dataTable4 = $("#mpexusage-prospectplustasks").DataTable({
				data: prospectplusTasksDataSet,
				pageLength: 250,
				order: [],
				layout: {
					topStart: {
			...
			// Create the pie chart for Call Force Tasks & Leads
			Highcharts.chart("container-prospectplus_progress", {
				chart: {
					type: "pie",
					backgroundColor: "#CFE0CE",
				},
			...
			//ProspectPlus - Count by Date Synced and Outcome
			var prospectplusLeadsCountBydateSyncedOutcomeSearch = search.load({
				type: "customer",
				id: "customsearch_pp_count_datesync_outcome",
			});

			prospectplusLeadsCountBydateSyncedOutcomeSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var oldProspectPlusSyncDate = null;
			var countProspectPlusSyncDateCount = 0;
			...
			prospectplusLeadsCountBydateSyncedOutcomeSearch
				.run()
				.each(function (illiciumLeadsCountBydateSyncedOutcomeSearchResultSet) {
					var illiciumLeadCount = parseInt(
						illiciumLeadsCountBydateSyncedOutcomeSearchResultSet.getValue({
					...
					if (
						countProspectPlusSyncDateCount != 0 &&
						illiciumSyncDate != oldProspectPlusSyncDate
					) {
						var totalNoOutcomePercentage = parseInt(
							(total_no_outcome / total_leads) * 100
						);
						...
						prospectplusDateSyncedOutcomeDataSet.push([
							oldProspectPlusSyncDate,
							totalNoOutcomeCol,
							totalNotInterestedCol,
							totalNoAnswerCol,
							totalVoicemailCol,
					...
					oldProspectPlusSyncDate = illiciumSyncDate;
					countProspectPlusSyncDateCount++;
					return true;
				});

			console.log(
				"countProspectPlusSyncDateCount: " + countProspectPlusSyncDateCount
			);
			if (countProspectPlusSyncDateCount > 0) {
				var totalNoOutcomePercentage = parseInt(
					(total_no_outcome / total_leads) * 100
				).toFixed(0);
				var totalNoOutcomeCol =
			...
			console.log(
				"prospectplusDateSyncedOutcomeDataSet" + prospectplusDateSyncedOutcomeDataSet
			);

			var dataTableProspectPlusDateSyncedOutcome = $(
				"#mpexusage-prospectplusDateSyncedOutcome"
			).DataTable({
				data: prospectplusDateSyncedOutcomeDataSet,
				pageLength: 250,
				order: [],
				responsive: true,
				layout: {
			...
			//ProspectPlus - Count by Outcome & Lead Status
			var prospectplusLeadsCountByOutcomeStatusSearch = search.load({
				type: "customer",
				id: "customsearch_pp_count_outcome_status",
			});

			prospectplusLeadsCountByOutcomeStatusSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var oldProspectPlusOutcome = null;
			var countProspectPlusSyncOutcomeCount = 0;
			...
			prospectplusLeadsCountByOutcomeStatusSearch
				.run()
				.each(function (illiciumLeadsCountByOutcomeStatusSearchResultSet) {
					var prospectCount = parseInt(
						illiciumLeadsCountByOutcomeStatusSearchResultSet.getValue({
					...
					if (
						countProspectPlusSyncOutcomeCount != 0 &&
						illiciumOutcome != oldProspectPlusOutcome
					) {
						prospectplusOutcomeStatusDataSet.push({
							callOutcome: oldProspectPlusOutcome,
							suspect_hot_lead: suspect_hot_lead,
							prospecy_quote_sent: prospecy_quote_sent,
							suspect_reassign: suspect_reassign,
							prospect_no_answer: prospect_no_answer,
					...
					oldProspectPlusOutcome = illiciumOutcome;
					countProspectPlusSyncOutcomeCount++;
					return true;
				});

			console.log(
				"countProspectPlusSyncOutcomeCount: " + countProspectPlusSyncOutcomeCount
			);
			if (countProspectPlusSyncOutcomeCount > 0) {
				prospectplusOutcomeStatusDataSet.push({
					callOutcome: oldProspectPlusOutcome,
					suspect_hot_lead: suspect_hot_lead,
					prospecy_quote_sent: prospecy_quote_sent,
			...
			console.log(
				"prospectplusOutcomeStatusDataSet" + prospectplusOutcomeStatusDataSet
			);

			outcomeStatusDataSet = [];
			if (!isNullorEmpty(prospectplusOutcomeStatusDataSet)) {
				prospectplusOutcomeStatusDataSet.forEach(function (preview_row, index) {
					var hotLeadPercentage = parseInt(
						(preview_row.suspect_hot_lead / preview_row.total_leads) * 100
					);
					var hotLeadCol =
			...
			var dataTableProspectPlusDateSyncedOutcome = $(
				"#mpexusage-prospectplusOutcomeStatus"
			).DataTable({
				data: outcomeStatusDataSet,
				pageLength: 250,
				order: [25, "desc"],
			...
			//ProspectPlus - Count by Sub-Category & Status
			var prospectplusLeadsCountBySubCategoryStatusSearch = search.load({
				type: "customer",
				id: "customsearch_pp_count_subcat_status",
			});

			prospectplusLeadsCountBySubCategoryStatusSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			var oldProspectplusSubCategory = null;
			var countProspectPlusSyncSubCategoryCount = 0;
			...
			prospectplusLeadsCountBySubCategoryStatusSearch
				.run()
				.each(function (callForceLeadsCountBySubCategoryStatusSearchResultSet) {
					var prospectCount = parseInt(
						callForceLeadsCountBySubCategoryStatusSearchResultSet.getValue({
					...
					if (
						countProspectPlusSyncSubCategoryCount != 0 &&
						callForceSubCategory != oldProspectplusSubCategory
					) {
						prospectplusSubCategoryStatusDataSet.push({
							callOutcome: oldProspectplusSubCategory,
							suspect_hot_lead: suspect_hot_lead,
							prospecy_quote_sent: prospecy_quote_sent,
							suspect_reassign: suspect_reassign,
							prospect_no_answer: prospect_no_answer,
					...
					oldProspectplusSubCategory = callForceSubCategory;
					countProspectPlusSyncSubCategoryCount++;
					return true;
				});

			console.log(
				"countProspectPlusSyncSubCategoryCount: " + countProspectPlusSyncSubCategoryCount
			);
			if (countProspectPlusSyncSubCategoryCount > 0) {
				prospectplusSubCategoryStatusDataSet.push({
					callOutcome: oldProspectplusSubCategory,
					suspect_hot_lead: suspect_hot_lead,
					prospecy_quote_sent: prospecy_quote_sent,
			...
			console.log(
				"prospectplusSubCategoryStatusDataSet" + prospectplusSubCategoryStatusDataSet
			);

			subCategoryStatusDataSet = [];
			if (!isNullorEmpty(prospectplusSubCategoryStatusDataSet)) {
				prospectplusSubCategoryStatusDataSet.forEach(function (preview_row, index) {
					var hotLeadPercentage = parseInt(
						(preview_row.suspect_hot_lead / preview_row.total_leads) * 100
					);
					var hotLeadCol =
			...
			var dataTableProspectPlusDateSyncedSubCategory = $(
				"#mpexusage-prospectplusIndustrySubCategoryStatus"
			).DataTable({
				data: subCategoryStatusDataSet,
				pageLength: 250,
				order: [25, "desc"],
			...
			//Sales Reporting - ProspectPlus - Completed Tasks
			var prospectplusCompletedTasksCurrentStatusSearch = search.load({
				type: "customer",
				id: "customsearch_pp_completed_tasks",
			});

			prospectplusCompletedTasksCurrentStatusSearch.filters.push(
				search.createFilter({
					name: "custrecord_salesrep",
					join: "CUSTRECORD_CUSTOMER",
					operator: search.Operator.NONEOF,
			...
			prospectplusCompletedTasksCurrentStatusSearch
				.run()
				.each(function (illiciumCompletedTasksCurrentStatusSearchResultSet) {
					var prospectCount = parseInt(
						illiciumCompletedTasksCurrentStatusSearchResultSet.getValue({
					...
					if (
						...
						prospectplusCompletedTasksDataSet.push({
							callOutcome: oldCompletedTaskDate,
							suspect_hot_lead: suspect_hot_lead,
							prospecy_quote_sent: prospecy_quote_sent,
							suspect_reassign: suspect_reassign,
			...
			console.log(
				"prospectplusCompletedTasksDataSet" + prospectplusCompletedTasksDataSet
			);

			completedTasksStatusDataSet = [];
			if (!isNullorEmpty(prospectplusCompletedTasksDataSet)) {
				prospectplusCompletedTasksDataSet.forEach(function (preview_row, index) {
					var hotLeadPercentage = parseInt(
						(preview_row.suspect_hot_lead / preview_row.total_leads) * 100
					);
					var hotLeadCol =
			...
			var dataTableProspectplusDateSyncedOutcome = $(
				"#mpexusage-prospectplusCompletedTasksCurrentStatus"
			).DataTable({
				data: completedTasksStatusDataSet,
				pageLength: 250,
				order: [],
			...
			.each(function (prospectListBySalesRepWeeklyResultSet) {
				...
				if (count1 == 0) {
					...
					total_leads =
						...
						prospecy_box_sent +
						suspect_pre_qualification +
						suspect_in_qualification + customer_shipmate_pending + suspect_zee_review + suspect_zee_rejected;
					...
					total_leads =
						...
						suspect_pre_qualification +
						suspect_in_qualification +
						customer_shipmate_pending + suspect_zee_review + suspect_zee_rejected;
				} else if (oldDate1 != null && oldDate1 != startDate) {
					debt_set2.push({
						dateUsed: oldDate1,
						suspect_hot_lead: suspect_hot_lead,
		...
		var dataTablePreview = $("#mpexusage-preview").DataTable({
			...
			columns: [
				...
				{
					title: "Suspect - Follow Up", //11
				},
				{
					title: "Suspect - LPO Follow Up", //12
				},
				{
					title: "Suspect - No Answer", //13
				},
				{
					title: "Suspect - In Contact", //14
				},
				{
					title: "Prospect - In Contact", //15
				},
				{
					title: "Suspect - Parking Lot", //16
				},
				{
					title: "Suspect - Lost", //17
				},
				{
					title: "Suspect - Out of Territory", //18
				},
				{
					title: "Suspect - Customer - Lost", //19
				},
				{
					title: "Prospect - Opportunity", //20
				},
				{
					title: "Prospect - Qualified", //21
				},
				{
					title: "Prospect - Box Sent", //22
				},
				{
					title: "Prospect - Quote Sent", //23
				},
				{
					title: "Customer - Free Trial Pending", //24
				},
				{
					title: "Customer - Free Trial", //25
				},
				{
					title: "Customer - ShipMate Pending", //26
				},
				{
					title: "Customer - Signed", //27
				},
				{
					title: "Total Lead Count", //28
				},
			],
			columnDefs: [
				{
					targets: [0, 3, 7, 21, 25, 26, 27],
					className: "bolded",
				},
			],
			footerCallback: function (row, data, start, end, display) {
				...
				$(api.column(28).footer()).html(total_lead);
			},
		});
		...
		for (var i = 0; i < data.length; i++) {
			...
			suspect_qualified[data[i][0]] = data[i][6];
			suspect_unqualified[data[i][0]] = data[i][7];
			suspect_zee_review[data[i][0]] = data[i][3];
			suspect_zee_rejected[data[i][0]] = data[i][4];
			suspect_validated[data[i][0]] = data[i][5];
			suspect_pre_qualification[data[i][0]] = data[i][8];
			suspect_in_qualification[data[i][0]] = data[i][9];
			suspect_reassign[data[i][0]] = data[i][10];
			suspect_follow_up[data[i][0]] = data[i][11];
			suspect_lpo_followup[data[i][0]] = data[i][12];
			suspect_no_answer[data[i][0]] = data[i][13];
			suspect_in_contact[data[i][0]] = data[i][14];
			prospect_in_contact[data[i][0]] = data[i][15];
			suspect_off_peak_pipeline[data[i][0]] = data[i][16];
			suspect_lost[data[i][0]] = data[i][17];
			suspect_oot[data[i][0]] = data[i][18];
			suspect_customer_lost[data[i][0]] = data[i][19];
			prospect_opportunity[data[i][0]] = data[i][20];
			prospect_qualified[data[i][0]] = data[i][21];
			prospecy_box_sent[data[i][0]] = data[i][22];
			prospecy_quote_sent[data[i][0]] = data[i][23];
			customer_free_trial_pending[data[i][0]] = data[i][24];
			customer_free_trial[data[i][0]] = data[i][25];
			customer_shipmate_pending[data[i][0]] = data[i][26];
			customer_signed[data[i][0]] = data[i][27];
			total_leads[data[i][0]] = data[i][28];
		}
		...
		Object.keys(prospecy_quote_sent).map(function (item, key) {
			...
			series_data33b.push(parseInt(suspect_zee_review[item]));
			series_data34b.push(parseInt(suspect_zee_rejected[item]));
		...
		plotChartPreview(
			...
			series_data29a,
			series_data30a,
			series_data31a, series_data32b, series_data33b, series_data34b
		);
		...
		if (role != 1000) {
			...
			if (!isNullorEmpty(zee_debt_set2)) {
				zee_debt_set2.forEach(function (preview_row, index) {
					...
					var suspectZeeReviewPercentage = parseInt(
						(preview_row.suspect_zee_review / preview_row.total_leads) *
						100
					);
					var suspectZeeReviewCol =
						preview_row.suspect_zee_review +
					...
					var suspectZeeRejectedPercentage = parseInt(
						(preview_row.suspect_zee_rejected / preview_row.total_leads) *
						100
					);
					var suspectZeeRejectedCol =
						preview_row.suspect_zee_rejected +
			...
			var dataTableZeePreview = $("#mpexusage-zee_overview").DataTable({
				...
				footerCallback: function (row, data, start, end, display) {
					...
					$(api.column(27).footer()).html(total_lead);
				},
			});
			...
			var zee_suspect_zee_review = [];
			var zee_suspect_zee_rejected = [];
			...
			for (var i = 0; i < zee_data.length; i++) {
				...
				zee_suspect_qualified[zee_data[i][0]] = zee_data[i][7];
				zee_suspect_unqualified[zee_data[i][0]] = zee_data[i][6];
				zee_suspect_zee_review[zee_data[i][0]] = zee_data[i][3];
				zee_suspect_zee_rejected[zee_data[i][0]] = zee_data[i][4];
				zee_suspect_validated[zee_data[i][0]] = zee_data[i][5];
				zee_suspect_pre_qualification[zee_data[i][0]] = zee_data[i][8];
				zee_suspect_in_qualification[zee_data[i][0]] = zee_data[i][9];
				zee_suspect_reassign[zee_data[i][0]] = zee_data[i][10];
				zee_suspect_follow_up[zee_data[i][0]] = zee_data[i][11];
				zee_suspect_lpo_followup[zee_data[i][0]] = zee_data[i][12];
				zee_suspect_no_answer[zee_data[i][0]] = zee_data[i][13];
				zee_suspect_in_contact[zee_data[i][0]] = zee_data[i][14];
				zee_prospect_in_contact[zee_data[i][0]] = zee_data[i][15];
				zee_suspect_off_peak_pipeline[zee_data[i][0]] = zee_data[i][16];
				zee_suspect_lost[zee_data[i][0]] = zee_data[i][17];
				zee_suspect_oot[zee_data[i][0]] = zee_data[i][18];
				zee_suspect_customer_lost[zee_data[i][0]] = zee_data[i][19];
				zee_prospect_opportunity[zee_data[i][0]] = zee_data[i][20];
				zee_prospect_qualified[zee_data[i][0]] = zee_data[i][21];
				zee_prospecy_quote_sent[zee_data[i][0]] = zee_data[i][22];
				zee_customer_free_trial_pending[zee_data[i][0]] = zee_data[i][23];
				zee_customer_free_trial[zee_data[i][0]] = zee_data[i][24];
				zee_customer_shipmate_pending[zee_data[i][0]] = zee_data[i][25];
				zee_customer_signed[zee_data[i][0]] = zee_data[i][26];
				zee_total_leads[zee_data[i][0]] = zee_data[i][27];
			}
			...
			var zee_series_data32a = [];
			var zee_series_data33a = [];
			...
			Object.keys(zee_total_leads).map(function (item, key) {
				...
				zee_series_data32a.push(parseInt(zee_suspect_zee_review[item]));
				zee_series_data33a.push(parseInt(zee_suspect_zee_rejected[item]));
			...
			plotZeeChartPreview(
				...
				zee_series_data28a,
				zee_series_data29a,
				zee_series_data30a, zee_series_data31a, zee_series_data32a, zee_series_data33a
			);
		}
			...
			.each(function (leadsListByDataCaptureStatusSearchResultSet) {
				...
				if (count1 == 0) {
					...
					total_leads =
						...
						prospecy_box_sent +
						suspect_in_qualification +
						suspect_pre_qualification + customer_shipmate_pending + suspect_zee_review + suspect_zee_rejected
				} else if (
		...
		if (!isNullorEmpty(datacapture_debt_set2)) {
			datacapture_debt_set2.forEach(function (preview_row, index) {
				...
				datacapture_overDataSet.push([
					...
					preview_row.suspect_zee_rejected, //4
					preview_row.suspect_qualified, //5
					preview_row.suspect_unqualified, //6
					preview_row.suspect_validated, //7
					preview_row.suspect_pre_qualification, //8
					preview_row.suspect_in_qualification, //9
					preview_row.suspect_reassign, //10
					preview_row.suspect_follow_up, //11
					preview_row.suspect_no_answer, //12
					preview_row.suspect_in_contact, //13
					preview_row.suspect_lpo_followup, //14
					preview_row.prospect_in_contact, //15
					preview_row.suspect_off_peak_pipeline, //16
					preview_row.suspect_lost, //17
					preview_row.suspect_oot, //18
					preview_row.suspect_customer_lost, //19
					preview_row.prospect_opportunity, //20
					preview_row.prospect_qualified, //21
					preview_row.prospect_box_sent, //22
					preview_row.prospecy_quote_sent, //23
					preview_row.customer_free_trial_pending, //24
					preview_row.customer_free_trial, //25
					preview_row.customer_shipmate_pending, //26
					preview_row.customer_signed, //27
					preview_row.total_leads, //28
					preview_row.lpoparentnameid, //29
				]);
		...
		var dataTableLPOPreview = $("#mpexusage-datacapture_overview").DataTable({
			...
			columns: [
				...
				{
					title: "Sales Rep ID", //29
				},
			],
			columnDefs: [
				{
					targets: [0, 3, 7, 22, 25, 26, 27, 28],
					className: "bolded",
				},
				{
					targets: [29],
		...
		var datacapture_suspect_zee_rejected = [];
		...
		for (var i = 0; i < datacapture_data.length; i++) {
			...
			datacapture_suspect_zee_rejected[datacapture_data[i][0]] =
			...
			datacapture_suspect_oot[datacapture_data[i][0]] = datacapture_data[i][18];
			...
			datacapture_customer_signed[datacapture_data[i][0]] =
				datacapture_data[i][27];
			datacapture_total_leads[datacapture_data[i][0]] = datacapture_data[i][28];
		}
		...
		var datacapture_series_data34a = [];
		...
		Object.keys(datacapture_total_leads).map(function (item, key) {
			...
			datacapture_series_data34a.push(
				parseInt(datacapture_suspect_zee_rejected[item])
			);
		...
		plotDataCaptureChartPreview(
			...
			datacapture_series_data31a, datacapture_series_data32a, datacapture_series_data33a, datacapture_series_data34a
		);
	...
	function plotDataCaptureChartPreview(
		...
		series_data29a,
		series_data30a,
		series_data31a, datacapture_series_data32a, datacapture_series_data33a, datacapture_series_data34a
	) {
		// console.log(series_data)
		...
		Highcharts.chart("container_datacapture_overview", {
			...
			series: [
				...
				{
					name: "Suspect - Rejected",
					data: datacapture_series_data34a,
					color: "#e86252",
					style: {
						fontWeight: "bold",
					},
