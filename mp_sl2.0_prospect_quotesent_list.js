...
define([
	...
	function onRequest(context) {
		...
		if (context.request.method === "GET") {
			...
			var sales_activity_notes =
				context.request.parameters.sales_activity_notes;
			...
			if (isNullorEmpty(sales_activity_notes)) {
				page_no = 2;
			}
			...
			form
				.addField({
					id: "custpage_sales_activity_notes",
					type: ui.FieldType.TEXT,
					label: "Page Number",
				})
				.updateDisplayType({
					displayType: ui.FieldDisplayType.HIDDEN,
				}).defaultValue = sales_activity_notes;
			...
			inlineHtml += userDropdownSection(
				userId,
				salesCampaign,
				custStage,
				source,
			...
			inlineHtml += parentLPODropdownSection(
				activeParentLPOSearchResultSet,
				context
			);
			...
			var custpage_salesrecordid =
				context.request.parameters.custpage_salesrecordid;
			...
			if (lostnoresponse == "true") {
				...
				var suiteletUrl =
					"https://1048144.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=395&deploy=1&compid=1048144&ns-at=AAEJ7tMQgAVHkxJsbXgGwQQm4xn968o7JJ9-Ym7oanOzCSkWO78&rectype=customer&template=154";
				suiteletUrl +=
					"&recid=" +
					customer_id +
					"&salesrep=" +
				...
				if (!isNullorEmpty(contact_email)) {
					email.send({
						...
						subject:
							...
							" " +
							custName +
							" - Do you want us to close your enquiry?",
						cc: [sales_rep_id],
				...
				var suiteletUrl =
					"https://1048144.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=395&deploy=1&compid=1048144&ns-at=AAEJ7tMQgAVHkxJsbXgGwQQm4xn968o7JJ9-Ym7oanOzCSkWO78&rectype=customer&template=148";
				suiteletUrl +=
					"&recid=" +
					customer_id +
					"&salesrep=" +
	...
	function dateFilterSection(
		...
		inlineHtml += '<div class="row">';
		inlineHtml +=
			'<div class="col-xs-12 heading1"><h4><span class="label label-default col-xs-12" style="background-color: #095C7B;font-size: 12px;border-radius: 30px;">DATE LEAD ENTERED - FILTER</span></h4></div>';
		inlineHtml += "</div>";
		inlineHtml += "</div>";
	...
	function userDropdownSection(
		...
		searchedSalesTeam.run().each(function (searchResult_sales) {
			...
			if (userId == employee_id && userId != 653718 && userId != 668711) {
				inlineHtml +=
					'<option value="' +
					employee_id +
					'" selected="selected">' +
		...
		inlineHtml += '<div class="form-group container cust_filter_section hide">';
		inlineHtml += '<div class="row">';
		inlineHtml +=
		...
		if (custStage == "1") {
			...
			if (custStatus == "42") {
				inlineHtml +=
					'<option value="42" selected>SUSPECT - QUALIFIED</option>';
			} else {
			...
			if (custStatus == "20") {
				inlineHtml +=
					'<option value="20" selected>SUSPECT - NO ANSWER</option>';
			} else {
			...
			if (custStatus == "69") {
				inlineHtml +=
					'<option value="69" selected>SUSPECT - IN CONTACT</option>';
			} else {
			...
			if (custStatus == "18") {
				inlineHtml +=
					'<option value="18" selected>SUSPECT - FOLLOW UP</option>';
			} else {
			...
			if (custStatus == "68") {
				inlineHtml +=
					'<option value="68" selected>SUSPECT - VALIDATED</option>';
			} else {
			...
			if (custStatus == "72") {
				inlineHtml +=
					'<option value="72" selected>PROSPECT - BOX SENT</option>';
			} else {
			...
			if (custStatus == "8") {
				inlineHtml +=
					'<option value="8" selected>PROSPECT - IN CONTACT</option>';
			} else {
		...
		inlineHtml +=
			'<select id="sales_campaign" class="js-example-basic-multiple js-states form-control" style="width: 100%" multiple="multiple">';
		inlineHtml += "<option></option>";
		...
		salesCampaignSearch.run().each(function (salesCampaignSearchResultSet) {
			...
			if (
				salesCampaignInternalId >= 69 ||
				salesCampaignInternalId == 67 ||
				salesCampaignInternalId == 62 ||
				salesCampaignInternalId == 70 ||
				salesCampaignInternalId == 77
			) {
				if (isNullorEmpty(salesCampaign)) {
					inlineHtml +=
						'<option value="' +
	...
	function parentLPODropdownSection(activeParentLPOSearchResultSet, context) {
		...
		inlineHtml +=
			'<select id="parent_lpo" class="js-example-basic-multiple js-states form-control" style="width: 100%" multiple="multiple">';
		inlineHtml += '<option value=""></option>';
		activeParentLPOSearchResultSet.each(function (activeParentLPOResultSet) {
			...
			if (isNullorEmpty(parentLPOInternalId)) {
				inlineHtml +=
					'<option value="' + parentLPOid + '">' + parentLPOName + "</option>";
			} else {
	...
	function tabsSection(
		...
		if (custStage == 1) {
			...
			var custListCommenceTodayResultsCount =
				custListCommenceTodayResults.runPaged().count;
			...
			inlineHtml +=
				'<div class="col-xs-12" style="text-align: center;font-size: 14px"><b>Total Lead Count: ' +
				custListCommenceTodayResultsCount +
				"</b></br> Pages: </div>";
